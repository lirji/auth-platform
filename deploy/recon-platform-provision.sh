#!/usr/bin/env bash
# 在统一 Casdoor 中幂等开通 recon-platform：organization、shared-app 回调、用户、角色(viewer/operator/admin)、
# 权限(recon.read/recon.dispose/recon.launch)。对齐 risk-platform-provision.sh，但 recon 无机器身份 / 无 SpiceDB。
# 密码必须由调用方通过 PASSWORD 注入；脚本不打印密码、client secret 或 access token。
#
# 用法：
#   RECON_USER=recon-e2e-admin PASSWORD='本地强口令' bash deploy/recon-platform-provision.sh
...
TENANT="recon-platform"
USER_NAME="${RECON_USER:?需要 RECON_USER（例如 recon-e2e-admin）}"
...
REDIRECT_URIS="${REDIRECT_URIS:-http://localhost:${RECON_UI_PORT}/auth/callback,http://127.0.0.1:${RECON_UI_PORT}/auth/callback}"
...
if [ -z "${USER_JSON}" ]; then
  api_post add-user "$(jq -nc --arg tenant "${TENANT}" --arg user "${USER_NAME}" --arg password "${PASSWORD_VALUE}" --arg app "${SHARED_APP}" '{owner:$tenant,name:$user,type:"normal-user",displayName:$user,email:($user+"@recon-platform.local"),phone:"",password:$password,signupApplication:$app,isAdmin:false,isForbidden:false,isDeleted:false,properties:{}}')"
fi
...
echo "==> 5. 重建 recon permissions（Casdoor 为 role->permission 真相源）"
# 角色累积:viewer⊂operator⊂admin。后端授权矩阵按这些 permission 判(见 CasdoorSecurityConfig)。
PERMISSION_MATRIX=$(cat <<'EOF'
recon.read|viewer,operator,admin
recon.dispose|operator,admin
recon.launch|admin
EOF
)
...
while IFS='|' read -r permission role_csv; do
  ...
  api_post add-permission "$(jq -nc --arg owner "${TENANT}" --arg name "${permission}" --argjson roles "${ROLES_JSON}" '{owner:$owner,name:$name,displayName:$name,model:"built-in/user-model-built-in",resourceType:"Custom",resources:["recon-platform"],actions:["Read"],effect:"Allow",roles:$roles,isEnabled:true}')"
...
printf '%s' "${CLAIMS}" | jq -e --arg tenant "${TENANT}" --arg cid "${CLIENT_ID}" --arg sub "${SUBJECT}" '
  ...
  and ((.permissions // []) | map(if type=="object" then .name else . end) | index("recon.read") != null)
  and ((.permissions // []) | map(if type=="object" then .name else . end) | index("recon.launch") != null)' >/dev/null

PERMISSION_COUNT="$(printf '%s' "${CLAIMS}" | jq '(.permissions // []) | length')"
JWKS_URI="${CASDOOR}/.well-known/jwks"
echo "✅ recon-platform 统一身份开通并验证完成"
echo "   tenant=${TENANT} user=${USER_NAME} sub=${SUBJECT} client_id=${CLIENT_ID} permissions=${PERMISSION_COUNT}"
echo ""
echo "   后端 secure profile env（填入 recon-batch 部署）："
echo "     SPRING_PROFILES_ACTIVE=secure"
echo "     RECON_AUTH_ISSUER=${CASDOOR}"
echo "     RECON_AUTH_JWK_SET_URI=${JWKS_URI}"
echo "     RECON_AUTH_ORG=${TENANT}"
echo "     RECON_AUTH_AUDIENCES=${CLIENT_ID}"
echo ""
echo "   前端 recon-console env（VITE_*，登录用；client 无 secret）："
