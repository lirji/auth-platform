...
if [ "$APPLY" = "1" ]; then
  ...
  if [ -n "$V_USER" ] && [ -n "$V_DOC" ]; then
    ...
    if [ "$SHIP" != "PERMISSIONSHIP_HAS_PERMISSION" ]; then
      echo "VERIFY FAIL: 成员 $V_USER 对 document:${TENANT}_${V_DOC} 的 view 未生效 (permissionship=${SHIP:-无响应})" >&2
    ...
    echo "VERIFY OK: 成员 $V_USER 经 D3 链对 document:${TENANT}_${V_DOC} 有 view —— 可安全用于 enforce。"
  else
    echo "VERIFY SKIP: 未提供 MEMBER_USERS 或 DOC_IDS，跳过 D3 生效校验（切 enforce 前建议至少给一组做门禁）。" >&2
  fi
else
