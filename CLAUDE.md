...
合并加载共 10 个 definition），权限用并集 + 层级箭头组合；公开链接用 `user:*` 通配主体；RBAC 作为特例
...
`casdoor-seed.sh`（role→scope）、`casdoor-tenant-provision.sh`（一键建 org+app+user+SpiceDB 成员组）、
`dept-authz-fixture.sh`（部门模型 seed/自校验）、`rag-authz-fixture.sh`（旧 D3 夹具，已被 dept 取代）。
> ⚠️ `spicedb-smoke.sh` 的部分断言基于**部门模型前**的 `document`（parent_space/public_viewer 继承），
> 未随 `knowledge.zed` 更新，直接跑会有断言失败；判部门模型请用 `dept-authz-fixture.sh`。
...
- `auth-console`（React+Vite+TS 前端管控台）**已落地**（M1-M6 完成，`auth-console/` 含 `src/`/`dist/`/
  Dockerfile；`dev.sh` 一键起前后端+基建）。仍**未落地**的规划项：`server` 的 gRPC facade、"ZedToken 水位缓存"。
- 部门层级知识隔离模型的权威文档：`docs/authz-department-model.md`；Casdoor SSO 接入：`docs/统一登录平台接入手册.md`。
- `DepartmentSyncService` bean 已接线（`ReconcileJob` 一并对账），但目前只有 `GroupSyncService` 挂在
  `CasdoorSyncController POST /sync` 上，部门同步尚无独立 HTTP 触发端点（走 reconcile/启动对账）。
