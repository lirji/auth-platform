...
无 lint/format 工具；JUnit 测试覆盖关键校验路径（protocol/core/sdk/server/admin 五模块 20 个测试类、117 个 `@Test`，
`./mvnw test` 全绿，见下），端到端验证仍靠 `deploy/` 冒烟脚本。
...
**0718 再收紧（对称覆盖读写全路径）**：core `lookupResources` 缺/空 permissionship 抛、`writeRelationships`/
`deleteRelationships` 缺 `writtenAt/deletedAt.token` 抛（`requireToken`——无水位则 read-after-write 不可保证）、
流式响应含顶层 `error` 抛；server `check-bulk` 引擎响应缺某资源直接抛（不再 `Boolean.TRUE.equals(null)` 静默降级成 deny）。
...
# 测试：protocol/core/sdk/server/admin 五模块共 20 个测试类（117 @Test，./mvnw test 全绿）。代表：
# ProtocolValueObjectsTest、SpiceDbAuthzEngineTest（core 严格校验/token/stream）、RemoteAuthzEngine{,Http}Test、
# CheckAccessAspectTest、AuthzController{Facade,Watermark}Test、AuthzServerSecurityFilter{,Boundary}Test、
# ZedTokenWatermarkTest、AdminControllerTest（ADM01 两段审计）、{Group,Department}SyncServiceTest、
# SyncServiceBoundaryTest、CasdoorClientTest、CasdoorSyncControllerTest、ReconcileJobTest、CasdoorGroupIdsTest、
# {Jdbc,InMemory}AuditStoreTest。全量：./mvnw test；单测：
...
org 继承 built-in 的 navItems 菜单裁剪）、`casdoor-hide-business.sh`（隐藏 Casdoor 控制台「商业」菜单：
所有 org 的 navItems 设白名单，key 清单对齐 v3.115.0，升级新增菜单需同步补；`RESTORE=1` 恢复全菜单）、
...
- `auth-console`（React+Vite+TS 前端管控台）**已落地**（M1-M6 完成，`auth-console/` 含 `src/`/`dist/`/
  ...
  recsys 广告主作用域模型（`schemas/recsys.zed`，另库 :8544）、全项目补测（20 类/117 @Test）、ADM01 两段审计、
  fail-closed 响应校验收紧、企业 SSO 联邦接入指南已于 2026-07-18 落地。
- 部门层级知识隔离模型的权威文档：`docs/authz-department-model.md`；Casdoor SSO 接入：`docs/统一登录平台接入手册.md`；
  企业 IdP 联邦（上游 SSO 汇入）：`docs/企业SSO联邦接入指南.md`；平台能力/API 参考：`docs/平台能力总览.md`；
  新项目接入双侧清单：`docs/新项目接入指南.md`；容量/性能：`docs/性能与容量规划.md`。
